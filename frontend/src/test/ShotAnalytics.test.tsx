...
describe('ShotAnalytics Component', () => {
  ...
  describe('Achievements Hub Extraction', () => {
    it('should render an achievements hub shortcut', async () => {
      mockGet.mockResolvedValue({ data: mockHeatmapData });
      renderShotAnalytics();

      await waitFor(() => {
        expect(screen.getByText('🏆 Achievements Hub')).toBeInTheDocument();
      });
    });

    it('should not render the achievements analytics tab anymore', async () => {
      mockGet.mockResolvedValue({ data: mockHeatmapData });
      renderShotAnalytics();
    ...
    it('should update keyboard hints to 1-7', async () => {
      mockGet.mockResolvedValue({ data: mockHeatmapData });
      renderShotAnalytics();

      await waitFor(() => {
        expect(screen.getByText(/number keys 1-7/i)).toBeInTheDocument();
      });
    });
  });
